...
import { ABI_PARAMS, JSON_ABI, type Specification, specModes } from "./types";
...
const getPieces = (
    ...
    const resultList = decodeAbiParameters(
        // @ts-expect-error dynamic type complains
        abiParameters,
        encodedData,
    );

    return resultList.map((decodedVal, index) => {
        // @ts-expect-error type inference in the way
        // the casting is the expected format.
        // but we're only interested in the name.
...
const prepareResultForJSONABI = (e: Envelope): Envelope => {
    if (e.spec.mode === "json_abi") {
        try {
            ...
            const orderedNamedArgs: [string, unknown][] = [];
