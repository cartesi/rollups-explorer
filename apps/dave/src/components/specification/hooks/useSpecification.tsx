...
type UpdateSpecificationAction = Action<DbSpecification>;

const catchBuilder = (opt: AtomActionLifecycle) => (reason: Error) => {
    const errorMessage = `Error when trying to ${opt.actionName} specification: ${reason.message}`;
    console.error(errorMessage);

    if (isFunction(opt.onFailure)) {
        opt.onFailure(new Error(errorMessage));
    }
};

const finallyBuilder = (opt: AtomActionLifecycle) => () => {
    if (isFunction(opt.onFinished)) {
        opt.onFinished();
    }
};

export const repositoryAtom = atom(new SpecificationRepository());
...
export const specificationsAtom = atom<DbSpecification[] | undefined>(
    undefined,
);
...
const useActions = () => {
    ...
    const actions = {
        ...
        updateSpecification(spec: Specification, opt?: ActionLifecycle) {
            updateSpecification({
                payload: spec as DbSpecification,
